...
- **Proposed pre-freeze readiness conditions D1–D3 + 1** provide a neutral way to ask whether this case—or an alternative or complementary case—is sufficiently intelligible, boundedly extensible, mandate-aligned and legally determinate to be useful for stable comparison.
...
A technically interesting challenge list cannot compensate for a case that is unclear, unbounded, outside the mandate or unable to state the governing regime needed for the authority determinations it asks participants to test.
...
## 6. Proposed pre-freeze readiness conditions

The following are **proposed ideal pre-freeze conditions for discussion, not agreed FG-TIDA requirements**. They are intended to be usable for this candidate case, for alternative cases, or for several complementary cases. They describe desirable properties of a case before a stable or frozen validation boundary is considered; they do not imply that FG-TIDA should select one universal case.

### D1 — Multi-audience intelligibility

The same case should retain its meaning when explained to citizens and service users; policy and public decision makers; architects and standards contributors; and implementers and developers. Each audience should be able to identify, at its appropriate level, who acts for whom, what was delegated, what changed, which evidence matters and why an action proceeds, is limited, escalated or held. A case understandable only to its authors is a weak candidate for shared use.

### D2 — Bounded extensibility, outside the committed case

An ideal shared case should be able to grow, shrink or move across domains without silently redefining its core concepts. The broad variants below are **not part of the deliberately restricted commitment** and do not create additional workstreams. They remain visible only to test whether the same concepts can extend without changing meaning and to identify where another case would be preferable:
...
Following the Theme #16 authority-interface discussion, and especially [Pam Dixon’s clarification](https://github.com/FG-TIDA/themes/issues/16#issuecomment-5481112607), any mandate whose validity, standing, scope, revocation or act-time binding is being tested should have its **governing legal regime declared as a case fact**. The applicable regime factually determines what can and cannot be conferred, when standing may change and with what continuing effect.

This does **not** mean that every useful case must be multijurisdictional. A single-regime case can be a valid and useful case. Where the same structural core is tested under materially different governing regimes, the more precise representation is a **controlled family of regime-specific variants**, each with its own declared legal facts, rather than one supposed universal legal answer.

This +1 condition is an addition to the earlier D1–D3 readiness view, not a competing authority model. Authority origination, standing, composition, contextual applicability, revocation and act-time validity remain determinations of the relevant authority/provenance requirements layer; the case declares the facts needed to exercise those requirements.

Taken together, the working **3 + 1** readiness view is therefore:

1. **intelligible enough to be shared;**
2. **boundedly extensible enough to reveal what is invariant and where another case is needed;**
...
These conditions are deliberately neutral among cases. The current logistics case is one working object against which they can be tested; sector-specific production cases, alternative cases or several complementary frozen cases can be assessed against the same ideal conditions.
...
A possible collaborative pre-freeze sequence is:

1. test whether a candidate case is useful to one or more existing themes;
2. test D1 across the relevant audiences;
...
This sequence is illustrative rather than a proposed governance procedure. It does not require agreement on one case before other cases can be developed. The current case can continue to mature while alternative or complementary cases are proposed, and the same readiness conditions can help compare their usefulness without requiring them to share one domain or one solution architecture.
...
- Reframed D1–D3 as **proposed ideal pre-freeze readiness conditions for discussion**, applicable equally to the current case, alternative cases or several complementary cases, rather than as requirements owned by this case.
...
- Updated the illustrative collaborative freeze path so the governing regime is declared before authority-related facts are frozen.
