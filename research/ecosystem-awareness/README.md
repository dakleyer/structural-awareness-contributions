...
### Case-study extensibility layer

The six reference scenarios are now also maintained as **bounded Model Case Studies** rather than one-off narratives. [A25 — Failure Case-Study Extensibility & Requirements-Conformance Transfer](./baseline/00K_A25_FAILURE_CASE_STUDY_EXTENSIBILITY_AND_CONFORMANCE_TRANSFER_v0.1.md) reuses the DAOS Annex-II discipline: **upward/vertical** extension adds actors/layers/scale, **downward** extension reduces to the smallest implementation that still preserves the structural failure, and **horizontal** extension changes domain while preserving the kernel.

Each family has an explicit profile: [00E](./baseline/00E_CASE_STUDY_EXTENSIBILITY_PROFILE_v0.1.md), [00F](./baseline/00F_CASE_STUDY_EXTENSIBILITY_PROFILE_v0.1.md), [00G](./baseline/00G_CASE_STUDY_EXTENSIBILITY_PROFILE_v0.1.md), [00H](./baseline/00H_CASE_STUDY_EXTENSIBILITY_PROFILE_v0.1.md), [00I](./baseline/00I_CASE_STUDY_EXTENSIBILITY_PROFILE_v0.1.md) and [00J](./baseline/00J_CASE_STUDY_EXTENSIBILITY_PROFILE_v0.1.md). The profiles freeze a structural kernel, inherited S/T route, same-family boundary and conformance-transfer statement. **00G explicitly includes a bounded multi-turn conversational-LLM/sycophantic-drift reduction** when repeated paraphrases, memory or derived assistant state lose source dependence and displace a still-valid task/frame; one-turn agreeableness alone is explicitly out of family.

The current theorem is conditional: if a proposed implementation passes the A25 family-admission tests — including failure reflection and conformance preservation — and actually satisfies the inherited canonical requirement route, then it cannot exhibit the **same structural family failure**. The listed cross-domain examples are first-pass structural extensions, not executed proof that every superficially similar implementation belongs to the family.
