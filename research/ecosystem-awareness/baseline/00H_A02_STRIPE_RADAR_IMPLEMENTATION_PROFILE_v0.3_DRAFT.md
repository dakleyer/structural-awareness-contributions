...
| **Companion trajectory** | [00H-A01 — Claude Agent SDK v0.3 Draft](./00H_A01_CLAUDE_AGENT_SDK_IMPLEMENTATION_PROFILE_v0.3_DRAFT.md) |
| **Predecessor** | [v0.2 Draft](./00H_A02_STRIPE_RADAR_IMPLEMENTATION_PROFILE_v0.2_DRAFT.md) · [v0.1 Draft](./00H_A02_STRIPE_RADAR_IMPLEMENTATION_PROFILE_v0.1_DRAFT.md) |
...
**v0.3 delta.** Aligns the strong Stripe/merchant-control trajectory with 00H v0.4 V19/V20. RADAR-H1 remains deliberately strong: Radar native risk/history controls, authoritative workload identity, current case/grant lookup, pre-refund gate and external cumulative ledger. The harder branch distributes a common unauthorized root campaign across many **individually valid leaf grants**. A per-agent/per-mandate ledger can therefore be correct locally while under-composing the common campaign. The fair strengthened peer, RADAR-H1-L, may add authoritative campaign/delegation lineage; if it passes U/G/I at equal/lower burden, EP differentiation is weakened.
...
**Status:** source-reviewed v0.3 adjacent strong-control implementation-trajectory draft aligned to 00H v0.4; unexecuted; no product-failure claim, benchmark result or comparative-superiority claim.
