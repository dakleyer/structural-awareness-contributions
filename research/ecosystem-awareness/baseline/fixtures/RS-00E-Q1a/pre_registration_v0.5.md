# Pre-Registration Record — RS-00E-Q1a Stage-0 Fixture Family — v0.5
...
| Item | Declaration |
| --- | --- |
| Fixture family ID / version | `RS-00E-Q1a` / v0.5 |
...
Repeat the same injected-loss fixture with the qualifier-loss detector/hook disabled or replaced by a no-op. This negative-control self-test MUST return `FAIL`. If it returns `PASS`, the instrumentation test is non-discriminating and no candidate run is admissible.

### 9.2 Canonical Trace v1 determinism check

The evidence-bearing trace is serialized under Canonical Trace v1:

- UTF-8 JSON;
- lexicographically sorted object keys;
- semantic arrays retain order; declared set-like collections are sorted by documented stable key;
- repository/file paths normalized to repository-relative POSIX form;
- closed excluded-field set: `run_id`, `wall_clock_timestamp`, `host_process_id`, `temporary_output_path`;
- integer JSON numbers only; non-integral measurements are normalized decimal strings; NaN/Infinity forbidden;
- no insignificant whitespace;
- `canonicalization_version = "CTv1"`;
- SHA-256 over the exact canonical bytes.

The reference helper is `canonical_trace_v1.py` and the corresponding unit test is `test_canonical_trace_v1.py` in this fixture directory.

Two executions of the same frozen bundle and configuration must produce byte-identical Canonical Trace v1 bytes after the declared exclusions. Their SHA-256 hashes must match. A mismatch is a **harness failure**.
...
| `B1` reaches a permitted posture in `P1` **and** `P2` within tolerance | **Descriptive negative observation only.** It is not a differential falsifier or an EA result under this record. That differential condition is reserved for a later record with a named comparator defender and independent review. |
| `B1` passes `P1` but fails `P2` while `B3` passes both | **Reserved differential condition.** Under v0.5, record the two configurations' behaviour descriptively only; a later record with a named comparator defender and independent review may assess a bounded differential. |
...
**Pre-registered expectation, recorded before execution:** `Q1a-P1` is expected to be non-discriminating, because the dependency is stated in the received field and any configuration that compares two strings can preserve it. `Q1a-P2` is the discriminating arm, but the frozen B1 provenance-and-registry-join peer is expected to resolve it and reach a permitted result. That is a pre-registered **descriptive** expectation only, not an EA differential result. Because no comparator defender or independent reviewer is named in this record, the v0.5 run remains descriptive only: neither P2 nor any other branch can support a differential conclusion. Recording these expectations in advance prevents a later reading of either primary arm as a positive result.
