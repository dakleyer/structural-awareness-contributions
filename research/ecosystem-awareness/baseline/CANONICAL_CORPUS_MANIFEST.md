...
- [`baseline/00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.2_DRAFT.md`](./00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.2_DRAFT.md) — latest working 00H successor for C12 / DBC-C05. v0.2 Draft preserves the Solstice Retail narrative and Q0–Q5 architecture while adding the concrete `00H-MAT-1` rule, discovery-input boundary, service/API-versus-agent-mandate separation, explicit control-evidence states, bounded 5+2 business-day authority closure, `REJECT ≠ finding invalid`, own-grant-staleness and non-material controls, stronger maker-checker/campaign peers, gate×variant coverage, and the H-00H-A / H-00H-B causal split. [`v0.1`](./00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.1.md) remains preserved as predecessor. It is not W3-admitted, executed, adopted or validated.
  - [`baseline/00H_A01_CLAUDE_AGENT_SDK_IMPLEMENTATION_PROFILE_v0.1_DRAFT.md`](./00H_A01_CLAUDE_AGENT_SDK_IMPLEMENTATION_PROFILE_v0.1_DRAFT.md) — source-reviewed Claude Agent SDK implementation-trajectory draft for 00H. It credits the SDK's pre-action hook and subagent/context surfaces, uses an authoritative external grant/case store for the defended peer, allows external cumulative ledgers, and treats subagent/compaction boundaries as falsifiable transformations rather than presumed failures.
  - [`baseline/00H_A02_STRIPE_RADAR_IMPLEMENTATION_PROFILE_v0.1_DRAFT.md`](./00H_A02_STRIPE_RADAR_IMPLEMENTATION_PROFILE_v0.1_DRAFT.md) — source-reviewed Stripe Radar / strong merchant-control trajectory draft for 00H. It credits Radar's real payment-risk/history counters, explicitly corrects that Radar transaction rules are documented for Charges/PaymentIntents/SetupIntents rather than as an inline Refund authorization gate, and permits a merchant pre-refund mandate/ledger layer to attempt the V11 falsifier fairly.
