...
- [`baseline/00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.1.md`](./00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.1.md) — synthetic C12 / DBC-C05 reference scenario ("The Quiet Four Thousand") for a materially qualified but currently unauthorized opportunity. It freezes the beneficial-opportunity/materiality rule before execution, separates non-adversarial aggregate-authority failure from deliberate cap decomposition, uses namespaced DBC disposition / AuthorityResponse semantics, and defines Q0–Q5 quality gates with local 00H-A0…A3 comparison arms. It is a candidate reference/test plan, not W3-admitted, executed, adopted or validated.
