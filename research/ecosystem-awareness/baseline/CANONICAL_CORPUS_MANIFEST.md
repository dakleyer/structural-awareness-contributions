...
- `baseline/01C_EA_REGIME_AWARENESS_INTERFACE_ANNEX_v0.2.md` — current additive EA↔Regime Awareness interface successor. It preserves the public minimal-detector boundary and v0.1 source contracts while adding the broader qualified `Δ_RA=[A_RA,B_RA,C_RA,D_RA]` projection — direction, confidence/intensity, current-capability frontier and residual — used by EA/MSCA positioning. It is **not** an extra frozen baseline document, implemented common API, independent empirical validation or claim that the broader delta semantics are published results of the minimal-detector paper. `01C_EA_REGIME_AWARENESS_INTERFACE_ANNEX_v0.1.md` remains preserved provenance.
...
- `baseline/01H_PARTICIPANT_LOCAL_ECOSYSTEM_POSITIONING_AND_DECISION_SCOPED_EPISTEMIC_OPPORTUNITY_v0.1.md` — additive working annex published 21 September 2026. It reconciles existing individual-adaptation, signalling-without-required-cooperation, Semantic Window, EHD, APQ/F9 and EA↔MSCA mechanisms into an explicit participant-local distributed reading; uses the reconciled qualified-position semantics with B as confidence/intensity; and preserves Decision-Scoped Epistemic Opportunity as the precursor to, not a competing definition of, the later canonical agentic-gradient law. It is **not** a new frozen baseline document, implemented protocol, convergence result, standards adoption or proof of benefit.
...
- `baseline/01J_ECOSYSTEM_SIGNALLING_SELECTIVE_DISCLOSURE_DISTRIBUTED_OPPORTUNITY_v0.1.md` — earlier additive signalling formulation retained as predecessor/provenance. It contains the historical local opportunity-gradient wording that preceded the current separation between Regime Awareness delta and downstream agentic gradient; it is not the current signalling route.
- `baseline/01J_ECOSYSTEM_SIGNALLING_SELECTIVE_DISCLOSURE_AND_CHOREOGRAPHED_REPOSITIONING_v0.1.md` — current additive working signalling annex published 23 September 2026. It defines selective participant-local signal projections across epistemic state, MSCA, ACC and authority/delegation; keeps signalling optional unless an applicable profile requires disclosure; supplies qualified inputs to the downstream agentic-gradient law and choreography; and introduces a falsifiable false-context / mission-displacement reference scenario with matched S0–S3 comparison. It is **not** a mandatory signalling protocol, common ecosystem contract, common MSCA, global gradient, convergence result, completed experiment or standards adoption.
...
- [`baseline/00_CANONICAL_REQUIREMENTS_CHALLENGES_SUFFICIENCY_HYPOTHESES_KPIS.md`](./00_CANONICAL_REQUIREMENTS_CHALLENGES_SUFFICIENCY_HYPOTHESES_KPIS.md) — the sole canonical requirements document in the current reader route: complete S1–S14 requirement taxonomy, T1–T4 sufficiently-good conditions, foundational H1–H6 and their KPI/falsification protocol. It is not an EA differential or a market result.
- [`baseline/00K_SIX_PRINCIPLE_REQUIREMENTS_MAPPING_AND_ABLATION_TEST_v0.1_DRAFT.md`](./00K_SIX_PRINCIPLE_REQUIREMENTS_MAPPING_AND_ABLATION_TEST_v0.1_DRAFT.md) — working ablation design that partitions S1–S14 across six operational principle families and pre-registers one leave-one-principle-out failure route for each 00E–00J scenario; designed, not executed.
...
- [`baseline/00E_FAILURE_MODES_100M_TOKEN_COMPOUNDING_v0.1.md`](./00E_FAILURE_MODES_100M_TOKEN_COMPOUNDING_v0.1.md) — technology-neutral enterprise reference failure scenario and integrated quality plan. §9A now carries a technology-agnostic external-corroboration addendum reviewed 24 Sep 2026 (empirical AI studies + official neighboring incident investigations); it is plausibility evidence, not product-failure evidence. Its two latest working product-profile successors apply the frozen case to [Microsoft Agent 365 v0.2 Draft](./00E_A01_MICROSOFT_AGENT_365_IMPLEMENTATION_PROFILE_v0.2_DRAFT.md) and [LangGraph/LangSmith v0.2 Draft](./00E_A02_LANGGRAPH_LANGSMITH_IMPLEMENTATION_PROFILE_v0.2_DRAFT.md); each preserves and links its public v0.1 predecessor.
- [`baseline/00F_FAILURE_MODE_SMART_CITY_MOBILITY_SYSTEMIC_DIVERGENCE_v0.1.md`](./00F_FAILURE_MODE_SMART_CITY_MOBILITY_SYSTEMIC_DIVERGENCE_v0.1.md) — technology-neutral smart-city mobility reference failure scenario in which residual uncertainty leads locally understandable Emergency Plan A, Emergency Plan B, NORMAL and HOLD postures to compete for one corridor; Q0–Q5 test whether their incompatibility is detected, bounded and requalified in time. §9A now carries a technology-agnostic external-corroboration addendum reviewed 24 Sep 2026 (official automation/systemic incidents + NIST digital-twin credibility work); it is plausibility evidence, not product-failure evidence. Its two latest working product-profile successors apply the frozen scenario to [FIWARE NGSI-LD / Orion-LD v0.2 Draft](./00F_A01_FIWARE_NGSI_LD_IMPLEMENTATION_PROFILE_v0.2_DRAFT.md) and [AWS IoT TwinMaker / IoT Core v0.2 Draft](./00F_A02_AWS_IOT_TWINMAKER_CORE_IMPLEMENTATION_PROFILE_v0.2_DRAFT.md), preserving their v0.1 predecessors while adding current quality-plan execution synchronization.
- [`baseline/00G_FAILURE_MODE_COLLECTIVE_FALSE_CONTEXT_CONVERGENCE_v0.4.md`](./00G_FAILURE_MODE_COLLECTIVE_FALSE_CONTEXT_CONVERGENCE_v0.4.md) — **canonical working 00G reference** for collective false-context convergence / the "Bar-to-Napoleon" cascade. v0.4 integrates the paired opaque false/genuine-regime-change fixture, corrected A/B/C/D semantics, source-dependence graph, strong comparator arms, bounded termination/re-entry, DBC-namespaced Q0–Q5 dispositions, explicit S1 authority-current-applicability coverage, DBC-C04/C06 routing, numerator/denominator/branch-oracle KPI instrumentation, seven non-normative visual aids, the complete source-reviewed OpenAI OAI-G0/G1/G2 implementation trajectory, and **§13A External Corroboration Addendum** (reviewed 24 Sep 2026) mapping documented sycophancy, conformity, consensus-collapse and evaluation-blind-spot evidence to 00G without treating those sources as proof of the literal fixture. [`v0.3 Draft`](./00G_FAILURE_MODE_COLLECTIVE_FALSE_CONTEXT_CONVERGENCE_v0.3_DRAFT.md), [`v0.2`](./00G_FAILURE_MODE_COLLECTIVE_FALSE_CONTEXT_CONVERGENCE_v0.2.md), [`v0.1`](./00G_FAILURE_MODE_COLLECTIVE_FALSE_CONTEXT_CONVERGENCE_v0.1.md) and the standalone [`00G-A01 Draft`](./00G_A01_OPENAI_AGENTS_STACK_IMPLEMENTATION_PROFILE_v0.1_DRAFT.md) remain lineage only. It is unexecuted, not W3-admitted, not integrated into a completed 00D execution and not a claim of hallucination immunity or OpenAI product failure.
  - **Integrated OpenAI profile:** now contained in 00G v0.4 §17, using OAI-G0 out-of-the-box/standard competent, OAI-G1 defended top-notch and OAI-G2 the same frozen top route under latent source/authority/context-transformation change. The standalone 00G-A01 v0.1 Draft is retained only as historical lineage.
- [`baseline/00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.5_DRAFT.md`](./00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.5_DRAFT.md) — latest working 00H successor for C12 / DBC-C05. **"The Quiet Four Thousand"** is the primary no-attacker reader frame: a good-faith one-case agent discovers the genuine ~4,000-customer finding and the system must avoid both unauthorized cross-case execution and silent loss of the material remainder. **"The Refund Campaign Nobody Approved"** is the V19/V20 adversarial companion: a compromised outsourced CRM/helpdesk Dispatcher may legitimately route/delegate ordinary tickets while lacking authority for the common financial campaign, creating valid leaves under an unauthorized root. The draft preserves Q0–Q5, V14–V18 adaptive strong-peer stress, U/G/I controls, explicit S8 non-amplification/root-lineage checks and the **EA0 standard requirements-conforming route**. The current review finds no additional non-Requirements gate justified. [v0.4 Draft](./00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.4_DRAFT.md), [v0.3 Draft](./00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.3_DRAFT.md), [v0.2 Draft](./00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.2_DRAFT.md) and [v0.1](./00H_FAILURE_MODE_BATCH_OPPORTUNITY_BEYOND_AUTHORITY_v0.1.md) remain preserved predecessors. It is not W3-admitted, executed, adopted or validated.
  - [`baseline/00H_A01_CLAUDE_AGENT_SDK_IMPLEMENTATION_PROFILE_v0.4_DRAFT.md`](./00H_A01_CLAUDE_AGENT_SDK_IMPLEMENTATION_PROFILE_v0.4_DRAFT.md) — source-reviewed Claude Agent SDK implementation-trajectory draft for 00H. It credits the SDK's pre-action hook and subagent/context surfaces, uses an authoritative external grant/case store for the defended peer, allows external cumulative ledgers, and treats subagent/compaction boundaries as falsifiable transformations rather than presumed failures.
  - [`baseline/00H_A02_STRIPE_RADAR_IMPLEMENTATION_PROFILE_v0.4_DRAFT.md`](./00H_A02_STRIPE_RADAR_IMPLEMENTATION_PROFILE_v0.4_DRAFT.md) — source-reviewed Stripe Radar / strong merchant-control trajectory draft for 00H. It credits Radar's real payment-risk/history counters, explicitly corrects that Radar transaction rules are documented for Charges/PaymentIntents/SetupIntents rather than as an inline Refund authorization gate, and permits a merchant pre-refund mandate/ledger layer to attempt the V11 falsifier fairly.
- [`baseline/00I_FAILURE_MODE_SEMANTIC_TOCTOU_v0.2_DRAFT.md`](./00I_FAILURE_MODE_SEMANTIC_TOCTOU_v0.2_DRAFT.md) — latest working semantic-TOCTOU scenario, “The Patch That Undid the Fix”. It tests whether deferred technical validity is incorrectly treated as current semantic applicability after intervening repair/freeze/version changes; Q0–Q6 maps to existing canonical requirements and records check-to-act binding as a clarification candidate, not a new S/T/H family.
- [`baseline/00J_FAILURE_MODE_RIGHTS_PROVENANCE_INVERSION_v0.1_DRAFT.md`](./00J_FAILURE_MODE_RIGHTS_PROVENANCE_INVERSION_v0.1_DRAFT.md) — working rights-provenance inversion scenario, “The Author Pays for Their Own Work”. It freezes a derived-content lineage in which a valid generation/provenance record can be promoted into an unsupported downstream rights claim; Q0–Q5 is explicitly derived from S1–S14/T1–T4/H1–H6/KPI semantics, with separate misimplemented and requirements-conforming routes. It is not executed or W3-admitted.
  - [`baseline/00J_A01_PANODYSSEY_TEMS_RIGHTS_PORTABILITY_IMPLEMENTATION_PROFILE_v0.1_DRAFT.md`](./00J_A01_PANODYSSEY_TEMS_RIGHTS_PORTABILITY_IMPLEMENTATION_PROFILE_v0.1_DRAFT.md) — source-reviewed H0/H1/H2 implementation trajectory using the Panodyssey publisher-side rights stack and TEMS rights-portability boundary. H1 is an explicitly constructed defended peer that must first pass paired Q5 enforcement controls; H2 freezes H1 and holds the same final enforcement proposition constant while the external resolver/identifier/lineage evidence contract changes. Unexecuted; not a product-failure claim.
  - [`baseline/assets/00J/README.md`](./assets/00J/README.md) — public visual-aid index for 00J. It routes the four additive SVG reader aids embedded in the scenario: [lineage chain](./assets/00J/1_lineage_chain.svg), [evidence scopes](./assets/00J/2_evidence_scopes.svg), [Route N vs Route Q](./assets/00J/3_two_routes.svg), and [Q0–Q5 gate card](./assets/00J/4_gate_reference_card.svg). These visuals are explanatory only; they do not modify requirements, fixture facts, evidence or result status.
