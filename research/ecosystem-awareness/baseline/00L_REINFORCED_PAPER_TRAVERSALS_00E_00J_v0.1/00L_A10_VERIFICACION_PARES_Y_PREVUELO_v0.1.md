...
En esta revisión: **12 ramas emparejadas, 128 mutaciones de instrumentación y 3 valores de frontera pasan**. La salida imprime las tres disposiciones por rama y falla con código no cero si se rompe un par, un campo obligatorio o el umbral. No se instalaron dependencias ni se ejecutaron las suites `pytest` completas porque el entorno no contiene `pytest`. La discrepancia documental de recuentos A4 registrada en [A09](./00L_A09_PUENTE_TRAZAS_EVIDENCIA_SIMBOLICA_v0.1.md) tampoco se resuelve con este subset.
