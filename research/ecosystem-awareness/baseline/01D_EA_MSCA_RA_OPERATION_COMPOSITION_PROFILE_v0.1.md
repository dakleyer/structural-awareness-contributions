...
Each test is a **proposal**. No benchmark result, pilot pass or institutional validation is asserted. Existing [EA validation profiles](./VALIDATION_PROFILE_READING_NOTE.md) and the RA [paper's theoretical/empirical claim separation](https://tegrity.ai/minimalistic-regime-aware-early-warning-systems/) are inputs to an eventual test design, not evidence that these joint tests have run.
