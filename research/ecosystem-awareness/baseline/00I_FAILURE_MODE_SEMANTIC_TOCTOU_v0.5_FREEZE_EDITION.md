...
> **Refuerzo semántico ejecutado — 26/09/2026.** La [revisión A23](./fixtures/00K-FORMAL/requirement-sufficiency/SEMANTIC_BRIDGE_REVIEW.md) comprueba un control candidato que vincula la acción con la última versión cualificada de su base: registrar el cambio no basta. Las 5.460 secuencias son de un modelo serial acotado; no ejecutan la carrera check→act de V10 ni la deriva V11. [Aplicación y registro del artículo](./00L_REINFORCED_PAPER_TRAVERSALS_00E_00J_v0.1/00L_A16_APLICACION_PLAN_Y_ESTADO_SUFICIENCIA_v0.1.md).
...
**Case-study family / extensibility:** the database rollback is the **minimum concrete instantiation** of the [00I Semantic TOCTOU case-study family](./00I_CASE_STUDY_EXTENSIBILITY_PROFILE_v0.1.md). The family is implementation-neutral: queued scripts, maintenance orders, deployments, access changes and other delayed actions belong only when the same `qualified at t0 → material basis changes → old determination reused at t1` kernel is preserved under [A25](./00K_A25_FAILURE_CASE_STUDY_EXTENSIBILITY_AND_CONFORMANCE_TRANSFER_v0.1.md).
