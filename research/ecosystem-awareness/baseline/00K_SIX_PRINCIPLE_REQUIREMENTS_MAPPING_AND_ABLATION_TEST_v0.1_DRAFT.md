...
That paper result now has a runnable companion: [**00K-A4 / P4–00H executable harness**](./fixtures/00K-A4-P4-00H/README.md). The original reviewed 11-test source remains preserved; the second reviewed package is preserved unchanged as [v0.2](./fixtures/00K-A4-P4-00H-v0.2/README.md) and independently reproduces **21/21** tests with NM, A2-L and the four-arm comparison. The active browsable harness composes those additions with the separate U/G indistinguishability sweeps and passes **27/27**. This is deterministic symbolic execution, not a live runtime/product benchmark.
...
**Complete first-pass symbolic coverage.** [00K-A04](./00K_A04_SIX_PRINCIPLE_SYMBOLIC_EXECUTION_SUMMARY_v0.1.md) records executable harnesses for **all six principles**: P1/00J **11/11**, P2/00E **11/11**, P3/00F **13/13**, P4/00H **27/27** active composed harness, P5/00I **12/12**, and P6/00G **15/15** — **89 passing symbolic tests** in total. In every current fixture, the strongest passing repair reconstructs the removed semantic invariant; **no TRUE SUBSTITUTE has yet been found in the tested repair surfaces**. The count is regression coverage, not a scientific score.
...
### P4 — Qualification-preserving handoff and authority lineage

When state crosses participants or systems, the receiving side must retain the material qualification needed to interpret it: source, provenance, representation relation, purpose/scope/time, delegation limits, history and unresolved qualifiers. A technically valid leaf or downstream record must not silently acquire stronger meaning because its lineage was lost.
...
|---|---|---|---|---|---|---|
| **−P1** | S14 | 00J | unsupported rights-enforcement route reopens | Route Q blocks/requalifies unsupported enforcement | **No TRUE SUBSTITUTE found; typed evidence/proposition schema passes by reconstructing P1 semantics.** | **SYMBOLIC HARNESS EXECUTED — 11/11** |
| **−P2** | S4 | 00E | unbounded review/search or forced deadline closure reopens | bounded decision-relevant closure restored | **No TRUE SUBSTITUTE found; budgeted decision-relevant search passes by reconstructing P2 semantics.** | **SYMBOLIC HARNESS EXECUTED — 11/11** |
| **−P3** | S5 | 00F | unresolved/stale state can become determinate corridor posture | explicit unresolved state blocks/requalifies | **No TRUE SUBSTITUTE found; explicit unresolved/non-permission closure passes by reconstructing P3 semantics.** | **SYMBOLIC HARNESS EXECUTED — 13/13** |
| **−P4** | S8 | 00H | root→leaf authority is lost; campaign amplification/loss route reopens | lineage + non-amplification restored | **No TRUE SUBSTITUTE found; A2-L passes U/G/I/NM by reconstructing P4 semantics.** | **PAPER EXECUTED + SYMBOLIC HARNESS EXECUTED — reviewed v0.2 21/21; active composed harness 27/27** |
| **−P5** | S10 | 00I | stale-but-technically-valid action executes after material change | time-of-use requalification restored | **No TRUE SUBSTITUTE found; native generation/version compare passes only by reconstructing P5 semantics.** | **SYMBOLIC HARNESS EXECUTED — 12/12** |
| **−P6** | S9 | 00G | dependent/local claims become collective truth | false transition rejected; genuine change still accepted | **No TRUE SUBSTITUTE found; dependency-aware source-independence peer passes by reconstructing P6 semantics.** | **SYMBOLIC HARNESS EXECUTED — 15/15** |
