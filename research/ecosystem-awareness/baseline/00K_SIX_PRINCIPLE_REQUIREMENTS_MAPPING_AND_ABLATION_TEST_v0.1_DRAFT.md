...
| **Primary dependencies** | [00 — Canonical Requirements](./00_CANONICAL_REQUIREMENTS_CHALLENGES_SUFFICIENCY_HYPOTHESES_KPIS.md) · [02 — Epistemic Safety Principles & Control Matrix](./02_EPISTEMIC_SAFETY_PRINCIPLES_CONTROL_MATRIX_v0.4.part01.md) · [02B — Foundational Syntax Closure](./02B_FOUNDATIONAL_SYNTAX_CLOSURE_AND_P1_P6_NORMAL_FORM_PROOF_v0.1.md) |
...
> **Evidence boundary.** This document now has **complete first-pass deterministic symbolic execution coverage for P1–P6**, but the executable harnesses themselves do **not** establish universal minimality, live agent/product validation, or that Ecosystem Positioning is the only architecture capable of satisfying the principles. The separate [00K-A16 formal relative-independence proof](./00K_A16_FORMAL_RELATIVE_INDEPENDENCE_PROOF_P1_P6_v0.1.md) establishes model-relative logical independence / irredundancy only for the current formalized P1–P6 predicates and declared witness class. The executable harnesses remain small symbolic fixtures designed to make the branch logic falsifiable.
...
### A4 — Remove P4: qualification-preserving authority basis
...
**Ablation.** Keep decision qualification, bounded inquiry, anti-false-closure, action-time requalification and composition checks, but remove any requirement that the relying decision retain or obtain a current, decision-sufficient authority qualification for the composed action. Full lineage is not assumed necessary; the repairer may use any legitimate scoped authority mechanism.
...
**Current strongest-repair assessment.** Leaf RBAC, rate/amount caps, risk gates, static allow-lists and non-owner approval do not separate matched U/G. Full lineage passes, but the serious-repair audit also finds passing non-EA implementations that do **not** expose full delegation history: scoped capability/caveat, legitimate maker-checker and an opaque owner-side PDP permit. These results narrow P4 to the minimal invariant actually required by 00H: a current, receiver-verifiable, decision-sufficient and non-amplifying authority basis for the composed action. Full historical lineage is one implementation, not the universal requirement established by this fixture.

**Restoration test.** Restore the minimal P4 invariant only: Q2/Q5 must obtain enough current legitimate authority qualification to determine whether the actual composed action is permitted without amplification, preserve the finding and route legitimate re-contract/requalification when it is not. This may be achieved by lineage or by a scoped authoritative attestation/capability.
...
The executable canonical P6 test therefore uses the [matched-authority A6 fixture](./fixtures/00K-A6-P6-00G/README.md). The serious layer goes further: six distinct immediate sources are held equal across F/G while only their **transitive material roots** differ. Direct source-ID counting is exposed as partial; identity/org/confidence/time/content/quorum repairs cannot separate the hidden-common-root pair. The hardened canonical A6 surface is **74/74** and finds no TRUE SUBSTITUTE; dependency-graph/effective-root peers pass by reconstructing P6's material dependence/non-substitution semantics. See [00K-A12](./00K_A12_P6_TRANSITIVE_DEPENDENCY_AND_STRONGEST_REPAIR_AUDIT_v0.1.md).
...
The P4 qualification is also material: the serious-repair search shows that full receiver-side delegation history is stronger than 00H requires. The supported P4 invariant is the narrower current decision-sufficient authority qualification/non-amplification relation documented in [00K-A10](./00K_A10_P4_MINIMAL_AUTHORITY_BASIS_AND_LINEAGE_REFINEMENT_v0.1.md).
