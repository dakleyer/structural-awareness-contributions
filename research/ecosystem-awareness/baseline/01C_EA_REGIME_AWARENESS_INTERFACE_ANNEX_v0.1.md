...
| RA representation/context layer | Ψ, admitted H_t(m), T*_t status, I and invisible-change declaration | Scoped Δ/τ/ℓ and observable access; EA burden/freshness challenge | Context/representation qualification, data series and limitations; RA detector and EA F3/F4. |
...
3. **Detect present compatibility:** with admitted context, RA computes I, B, deviation and directional P_t above declared thresholds. The report keeps source, context, scope, delay and invisible-change qualifiers. Approximate instability is admissible; exact tipping prediction is not.
