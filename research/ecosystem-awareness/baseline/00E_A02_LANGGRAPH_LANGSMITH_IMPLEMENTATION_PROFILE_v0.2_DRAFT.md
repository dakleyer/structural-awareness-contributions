...
| **Status** | Additive successor draft · source-reviewed working draft · not a product ranking, certification or endorsement |
...
| **Q5 — enterprise composition**; S9/S11/S12/S14 → T1/T2/T4 → H1/H2/H3/H4/H6 | Parent/subgraphs, shared stores and end-to-end traces can make composition explicit | Correlates versions and dependencies and refuses unobserved branches | Compose only passed scopes. A success in one graph or domain cannot offset a failed gate elsewhere; preserve residual and reopen the exact failed scope. |
...
| **Q5** | S9/S11/S12/S14 composition preserves scope conflict/non-substitution and refuses synthesis across failed material gates | graph completion/auditable trace is mistaken for a qualified enterprise conclusion |

### KPI / disposition trace requirement

For each gate, the execution trace MUST bind the parent KPI numerator, denominator, branch oracle, threshold and disposition to the same `σ(d,t)`. LangSmith traces/evals are evidence substrates; they are not automatically the oracle. Missing denominator/oracle/required qualification produces `UNKNOWN`.

If a defended top LangGraph/LangSmith peer implements the same bounded requalification/non-substitution behavior at equal or lower burden, the proposed EA differential is weakened or falsified.
