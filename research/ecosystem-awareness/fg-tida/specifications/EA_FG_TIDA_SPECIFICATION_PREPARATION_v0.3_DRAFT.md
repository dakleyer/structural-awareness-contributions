...
**Status:** Draft v0.3 — audited specification-preparation control document  
...
**Predecessor:** [v0.2 dated preparation snapshot](./EA_FG_TIDA_SPECIFICATION_PREPARATION_v0.2_DRAFT.md)
...
**EA-CONF — Cross-Theme Conformance, Decision-Boundary Evaluation and Validation Profiles**
...
- a Decision Boundary Evaluation Profile for cross-Theme composition;
...
## 5A. Cross-Theme Decision Boundary & Evidence Profile

The Decision Boundary Challenge adds a candidate **test/conformance layer**, not a new Theme ontology.

Its FG-TIDA projection asks whether independently owned Theme outputs can meet at a material decision boundary while preserving:

- producer-native semantics;
- issuer and scope;
- determination/residual state;
- operating posture;
- admissibility and authority boundaries;
- human-oversight authority/capacity;
- separate execution/continuation outcome;
- revalidation conditions;
- burden and accountability evidence.
...
For v0.3 preparation, the profile is classified:

> **T — Test/conformance only**

It is **not** promoted into N1/N2 normative semantics merely because the profile is useful.

A future WG may later normatively require selected conformance properties, such as semantic preservation or authority non-creation, but only after ordinary source-owner, scope and conformance review.

### 5A.2 What the profile may compare

A future EA-CONF profile may compare named configurations under frozen facts, authority, resources and deadlines using:

1. hard semantic/authority/continuity/traceability gates;
2. per-measure rates and structured outcomes;
3. burden and accountability/reconstructability vectors;
4. challenger/defender status;
5. an evidence-strength ladder.

It should not produce a universal vendor score or allow one dimension to compensate invisibly for a failure in another.

### 5A.3 Evidence-strength ladder

The programme-independent DBC uses **DBC-EL0…DBC-EL5** to distinguish:

- defined;
- cross-theme mapped and contributor-reviewed;
- deterministic;
- executable comparative;
- interoperable / independently governed;
- independently replicated / applied.

This is a ranking of **evidence maturity for a scoped claim**, not a ranking of architecture quality.

### 5A.4 Challenger / defender role

For any comparative claim beyond descriptive evidence, the strongest relevant peer should, where practical, have an independent defender/challenger able to improve its configuration before freeze. A self-configured peer may still support descriptive evidence, but the claim ceiling is lower.

This extends the W2 comparator-defender discipline into FG-TIDA conformance work without forcing W2 and EA-CONF to become the same programme.
...
- EA-ITP-01 fixtures;
- UC-EA-01…04;
- Decision Boundary Challenge / FG-TIDA Decision Boundary Evaluation fixtures;
- benchmark arms;
...
### 12.4 Decision Boundary Evaluation Profile

The candidate EA-CONF layer should be able to test not only whether one interface record is syntactically/semantically conforming, but whether a **composition of independently owned outputs** remains correct at the decision boundary.

The current DBC profile preserves five separate layers:
...
A conformance profile MUST NOT silently map one layer into another.

The profile should also namespace collisions such as ESCALATE, which can denote:

- a producer-native verdict;
- a DBC procedural escalation;
- an authority owner's own escalation response.

Where comparison is performed, the profile should include:

- valid-continuity controls;
- explicit hard gates;
- per-measure rates;
- burden and reconstructability;
...
### 13.5 Current executable convergence route — UC-6 → UC-4

The public discussion on 23 September 2026 now provides a concrete conformance-development sequence.

**UC-6 semantic reference.**  
[Use Case #6](https://github.com/FG-TIDA/use-cases/issues/6) holds agent identity, grant, action, documents, recipient, human authority and other operating conditions constant while changing the declared purpose. It therefore provides a compact authority-applicability boundary test without introducing a new attack, capacity failure or grant mutation.

**Theme #13 worked mapping.**  
The current proposal is to apply the operational-risk / response-window / epistemic-opportunity matrix field by field to UC-6 A/B, recording actual inputs, sources, units/types, rules/thresholds, structured state, expected outputs, N/A fields and unresolved mappings.

**Theme #16 worked mapping.**  
The current proposal is to annotate the same case with the authority determination received, available intervention options, human decision and separate execution/continuation outcome. The base fixture keeps human-input authenticity and capacity constant, and H1 approval cannot expand G1.

**Semantic review before implementation.**  
Case and matrix contributors validate mappings before freeze; unresolved questions remain explicit rather than being filled by the test maintainer.

**UC-4 executable profile.**  
Nelson's public proposal is then to take the reviewed mapping into [Use Case #4](https://github.com/FG-TIDA/use-cases/issues/4) as a bounded, versioned executable profile: mapping, fixtures, traces and report against agreed expectations. UC-4 retains its federated signal/corroboration/containment core and imports the case through a versioned adapter.

This sequence is important because it separates:

    case facts / expected domain outcome
    → semantic mapping
    → cross-theme review
    → frozen adapter/profile
    → executable fixtures/traces
    → evidence/report

The test implementation therefore does not become the semantic owner.

**Public anchors**

- Theme #13 — Iván, deterministic UC-6 proposal: https://github.com/FG-TIDA/themes/issues/13#issuecomment-5798563483
- Theme #13 — Nelson, bounded UC-4 executable profile: https://github.com/FG-TIDA/themes/issues/13#issuecomment-5802619701
- Theme #16 — Nelson, common UC-6 cross-interface mapping: https://github.com/FG-TIDA/themes/issues/16#issuecomment-5803023066
...
9. **Conformance**
   ...
   - Decision Boundary Evaluation Profile
   - challenger/defender comparative procedure
   - evidence-strength classification and per-measure reporting
...
13. **Decision-boundary profile:** which DBC properties, if any, should become normative conformance requirements rather than remain test-only?
14. **Evidence maturity:** should a future WG adopt an evidence-strength ladder such as DBC-EL0…EL5, and under what editorial owner/process?
...
10. maintain the Decision Boundary Evaluation profile as test/conformance-only material and work the UC-6 → UC-4 route to an agreed versioned fixture/profile;
11. record challenger/defender and evidence-level metadata for any comparative DBC run;
12. keep all unresolved placement/editorial decisions explicit rather than solving them unilaterally.

## 21A. Audit disposition — v0.3

This v0.3 was audited against:
...
- Decision Boundary Challenge v0.2 and its FG-TIDA projection; and
- the 23 September UC-6 → UC-4 executable-profile convergence proposed publicly in Themes #13/#16.
...
| Decision Boundary / cross-theme evaluation | Added in v0.3 as T — test/conformance only | UC-6 mapping and UC-4 executable-profile agreement/execution still open |
| Challenger / evidence-strength discipline | Added in v0.3 | requires named defender/challenger for strongest comparative claims |
...
10. semantic-owner review of the UC-6 cross-theme mapping; and
11. agreement and execution of the bounded UC-4 imported profile.
...
This v0.3 preparation map reflects the source corpus and FG-TIDA application state available when it was drafted. It is intentionally **not self-updating** as the general architecture evolves.
...
The predecessor v0.2 remains a dated design record and should not be rewritten solely to make the historical draft appear current.
